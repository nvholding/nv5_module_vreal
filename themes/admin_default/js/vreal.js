...
// Xu ly real_type ---------------------------------------
function nv_chang_real_type(rid, mod) {
	var nv_timer = nv_settimeout_disable('id_' + mod + '_' + rid, 5000);
	var new_vid = $('#id_' + mod + '_' + rid).val();
	$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=change_real_type&nocache=' + new Date().getTime(), 'rid=' + rid + '&mod=' + mod + '&new_vid=' + new_vid, function(res) {
		var r_split = res.split('_');
		if (r_split[0] != 'OK') {
			alert(nv_is_change_act_confirm[2]);
		}
		clearTimeout(nv_timer);
		nv_show_list_real_type();
	});
	return;
}

function nv_show_list_real_type() {
	if (document.getElementById('module_show_list')) {
		$('#module_show_list').load(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=list_real_type&nocache=' + new Date().getTime());
	}
	return;
}

function nv_del_real_type(rid) {
	if (confirm(nv_is_del_confirm[0])) {
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=del_real_type&nocache=' + new Date().getTime(), 'rid=' + rid, function(res) {
				var r_split = res.split('_');
				if (r_split[0] == 'OK') {
					nv_show_list_real_type();
...
// Xu ly block ---------------------------------------
function nv_chang_real_block(bid, mod) {
	var nv_timer = nv_settimeout_disable('id_' + mod + '_' + bid, 5000);
	var new_vid = $('#id_' + mod + '_' + bid).val();
	$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=chang_real_block&nocache=' + new Date().getTime(), 'bid=' + bid + '&mod=' + mod + '&new_vid=' + new_vid, function(res) {
		var r_split = res.split('_');
		if (r_split[0] != 'OK') {
			alert(nv_is_change_act_confirm[2]);
		}
		clearTimeout(nv_timer);
		nv_show_list_real_block();
	});
	return;
}

function nv_show_list_real_block() {
	if (document.getElementById('module_show_list')) {
		$('#module_show_list').load(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=list_real_block&nocache=' + new Date().getTime());
	}
	return;
}

function nv_del_real_block(bid) {
	if (confirm(nv_is_del_confirm[0])) {
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=del_real_block&nocache=' + new Date().getTime(), 'bid=' + bid, function(res) {
			var r_split = res.split('_');
			if (r_split[0] == 'OK') {
				nv_show_list_real_block();
			} else if (r_split[0] == 'ERR') {
...
function nv_add_otherimage() {
	var newitem = "<div class=\"input-group margin-top-lg\"><input class=\"form-control\" type=\"text\" value=\"\" name=\"otherimage[]\" id=\"otherimage_" + file_items + "\"/>";
	newitem += "<span class=\"input-group-btn\"><button type=\"button\" class=\"btn btn-info\" name=\"selectfile\" onclick=\"nv_open_browse( '" + nv_base_adminurl + "index.php?" + nv_name_variable + "=upload&popup=1&area=otherimage_" + file_items + "&path=" + file_dir + "&type=file&currentpath=" + currentpath + "', 'NVImg', 850, 400, 'resizable=no,scrollbars=no,toolbar=no,location=no,status=no' ); return false; \"><i class=\"fa fa-image fa-lg\">&nbsp;</i></button></span></div>";
