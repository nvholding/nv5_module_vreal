...
$rid = $nv_Request->get_string('rid', 'get', 0);
...
// Get real_type
$sql = 'SELECT ' . NV_LANG_DATA . '_title as title, rid FROM ' . $db_config['prefix'] . '_' . $module_data . '_real_type';
$result = $db->query( $sql );

while( $row = $result->fetch() )
{
	$row['selected'] = ( $row['rid'] == $rid ) ? 'selected="selected"' : '';
	$xtpl->assign( 'ROW', $row );
	$xtpl->parse( 'form.loop_real_type' );
}
...
// Fetch Limit
$db->sqlreset()->select('DISTINCT COUNT(*)')->from($table_search . " " . $table_exchange)->where("t1.status =1 " . $search . " " . $show_price);
$sth = $db->prepare($db->sql());
$sth->execute();

$num_items = $sth->fetchColumn();

$db->select("DISTINCT t1.id, t1.listcatid, t1.publtime, t1." . NV_LANG_DATA . "_title, t1." . NV_LANG_DATA . "_alias, t1." . NV_LANG_DATA . "_hometext, t1." . NV_LANG_DATA . "_address, t1.homeimgalt, t1.homeimgfile, t1.homeimgthumb, t1.product_price, t1.product_discounts, t1.money_unit, t1.showprice, t1.product_unit, t2.exchange " . $sql_i)
    ->order($order_by)
    ->limit($per_page)
...
while (list($id, $listcatid, $publtime, $title, $alias, $hometext, $address, $homeimgalt, $homeimgfile, $homeimgthumb, $product_price, $product_discounts, $money_unit, $showprice, $product_unit) = $result->fetch(3)) 
{
	...
	$large_thumb = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $homeimgfile;
    $data_content[] = array(
		'id' => $id,
		'publtime' => $publtime,
		'title' => $title,
		...
		'cat_title' => $global_array_cat[$listcatid]['title'],			
		'cat_link' => $link . $global_array_cat[$listcatid]['alias'],		
