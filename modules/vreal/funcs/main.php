...
if( empty( $contents ) )
{
	...
	if( $pro_config['home_view'] == 'view_home_all' ) {
		...
		$db->select( 'id, listcatid, publtime, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, ' . NV_LANG_DATA . '_hometext, ' . NV_LANG_DATA . '_address, homeimgalt, homeimgfile, homeimgthumb, product_code, product_price, product_discounts, money_unit, showprice, product_unit, hitstotal' )->order( $orderby )->limit( $per_page )->offset( ( $page - 1 ) * $per_page );

		$result = $db->query( $db->sql() );

		while( list( $id, $listcatid, $publtime, $title, $alias, $hometext, $address, $homeimgalt, $homeimgfile, $homeimgthumb, $product_code, $product_price, $product_discounts, $money_unit, $showprice, $product_unit, $hitstotal ) = $result->fetch( 3 ) )
		{				
	...
	} elseif( $pro_config['home_view'] == 'view_home_cat' | $pro_config['home_view'] == 'view_home_catlist' ) {
		foreach( $global_array_cat as $catid_i => $array_info_i )
		{
			if( $array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0 )
			{
				...
				$db->select( 'id, publtime, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, ' . NV_LANG_DATA . '_hometext, ' . NV_LANG_DATA . '_address, homeimgalt, homeimgfile, homeimgthumb, product_code, product_price, product_unit, product_discounts, money_unit, showprice, hitstotal' )->order( 'id DESC' )->limit( $array_info_i['numlinks'] );

				$result = $db->query($db->sql());
				$data_pro = array();

				while( list( $id, $publtime, $title, $alias, $hometext, $address, $homeimgalt, $homeimgfile, $homeimgthumb, $product_code, $product_price, $product_unit, $product_discounts, $money_unit, $showprice, $hitstotal ) = $result->fetch( 3 ) )
				{
					$sql = $db->query( 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_units WHERE id = ' . $product_unit );
					$data_unit = $sql->fetch();
					$data_unit['title'] = $data_unit[NV_LANG_DATA . '_title'];
					...
					while( list( $bid, $title, $_alias, $_bid, $_id ) = $_result->fetch( 3 ) )				
					{
						$data_real_block[] = array(
							'real_block' => $title,
							'real_block_link' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $module_info['alias']['real_block'] . '/' . $_alias
						);				
					}					
				...
				$data_content[] = array(
					...
					'data' => $data_pro,
					'data_real_block' => $data_real_block,
					'num_pro' => $num_pro,
		...
		foreach( $global_array_group as $groupid_i => $array_info_i )
		{
			if( $array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0 )
			{
				...
				$db->select( 'id, listcatid, publtime, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, ' . NV_LANG_DATA . '_hometext, ' . NV_LANG_DATA . '_address, homeimgalt, homeimgfile, homeimgthumb, product_code, product_price, product_discounts, money_unit, showprice, hitstotal' )->order( 'id DESC' )->limit( $num_links );

				$result = $db->query( $db->sql() );

				$data_pro = array();

				while( list( $id, $listcatid, $publtime, $title, $alias, $hometext, $address, $homeimgalt, $homeimgfile, $homeimgthumb, $product_code, $product_price, $product_discounts, $money_unit, $showprice, $hitstotal ) = $result->fetch( 3 ) )
				{
					if( $homeimgthumb == 1 )//image thumb
					{
						$thumb = NV_BASE_SITEURL . NV_FILES_DIR . '/' . $module_name . '/' . $homeimgfile;
