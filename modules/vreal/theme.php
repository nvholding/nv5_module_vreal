		...
		foreach( $data_content as $data_row )
		{
			if( $data_row['num_pro'] > 0 )
			{
				...
				foreach( $data_row['data'] as $data_row_i )
				{
					...
					$xtpl->assign('TITLE0', nv_clean60( $data_row_i['title'], 40 ) );
					$xtpl->assign('IMG_SRC', $data_row_i['homeimgthumb']);
					$xtpl->assign('IMG_SRC_LARGE', $data_row_i['homeimgfile']);
					$xtpl->assign('LINK_ORDER', $data_row_i['link_order']);
					$xtpl->assign('height', $pro_config['homeheight']);
					$xtpl->assign('width', $pro_config['homewidth']);
					$xtpl->assign('hometext', $data_row_i['hometext']);
					...
					foreach( $data_row['data_real_block'] as $data_real_block_i ) {
						$xtpl->assign('REAL_BLOCK', $data_real_block_i['real_block']);
						$xtpl->assign('REAL_BLOCK_LINK', $data_real_block_i['real_block_link']);
						$xtpl->parse('main.catalogs.items.real_block');	
					}						

					if( $pro_config['active_price'] == '1' ) {
						if( $data_row_i['showprice'] == '1' ) {
							$xtpl->assign('product_price', nice_number( $data_row_i['product_price']) . ' ' . $data_row_i['money_unit'] . '/' . $data_row_i['product_unit']);
							$xtpl->parse('main.catalogs.items.price' );
							$xtpl->parse('main.catalogs.items.price1' );
						}
						else {
							$xtpl->parse('main.catalogs.items.contact' );
							$xtpl->parse('main.catalogs.items.contact1' );
						}
					}
...
{
	global $module_info, $lang_module, $module_file, $pro_config, $op, $array_displays, $module_data, $db, $db_config;
	...
	if( ! empty( $data_content ) )
	{
		...
		foreach( $data_content as $data_row ){
			...
			$xtpl->assign('IMG_SRC_LARGE', $data_row['thumb_large']);			
			...
			$xtpl->assign('hitsotal', $data_row['hitstotal']);	
			$xtpl->assign('publtime', nv_date('d/m/Y', $data_row['publtime']));	
			
			$xtpl->assign('num', $num_row );

			$db->select('t1.bid, t1.' . NV_LANG_DATA . '_title As title, t1.' . NV_LANG_DATA . '_alias As alias, t2.bid, t2.id')
				->from($db_config['prefix'] . '_' . $module_data . '_real_block t1')	
				->join('INNER JOIN ' . $db_config['prefix'] . '_' . $module_data . '_block t2 ON t2.bid = t1.bid')	
				->where('t2.id = ' . $data_row['id']);
			$_result = $db->query($db->sql());	
			$data_real_block = array();				
			while( list( $bid, $_title, $_alias, $_bid, $_id1 ) = $_result->fetch( 3 ) ) {
				$data_real_block[] = array(
					'real_block' => $_title,
					'real_block_link' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_data . '&amp;' . NV_OP_VARIABLE . '=' . $module_info['alias']['real_block'] . '/' . $_alias
				);				
			}	
			
			foreach( $data_real_block as $data_real_block_i ) {
				$xtpl->assign('REAL_BLOCK', $data_real_block_i['real_block']);
				$xtpl->assign('REAL_BLOCK_LINK', $data_real_block_i['real_block_link']);
				$xtpl->parse('main.items.real_block');	
				$xtpl->parse('main.items.real_block1');	
			}	
			
			if (!empty($data_row['author'])) {
				$xtpl->assign('author', $data_row['author']);	
				$xtpl->parse('main.items.author');
				$xtpl->parse('main.items.author1');
			}
			...
			if( $pro_config['active_price'] == '1' ) {
				if( $data_row['showprice'] == '1' ) {	
					$data_row['money_unit'] = ( $data_row['money_unit'] != "" ) ? $data_row['money_unit'] : "N/A";
					$xtpl->assign('product_price', nice_number( $data_row['product_price']) . ' ' . $data_row['money_unit'] .'/' . $data_row['product_unit']);
...
function view_search_all( $data_content, $html_pages = "" )
{
	...
	$xtpl = new XTemplate( "search_result.tpl", NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/" . $module_file );
	$xtpl->assign('LANG', $lang_module );
	$xtpl->assign('TEMPLATE', $module_info['template']);
	$xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL );
	...
	if( ! empty( $data_content ) )
	{
		...
		foreach( $data_content as $data_row )
		{
			...
			$xtpl->assign('TITLE0', nv_clean60( $data_row['title'], 40 ) );
			$xtpl->assign('IMG_SRC', $data_row['homeimgthumb']);
			$xtpl->assign('IMG_SRC_LARGE', $data_row['large_thumb']);			
			$xtpl->assign('LINK_ORDER', $data_row['link_order']);
			$xtpl->assign('height', $pro_config['homeheight']);
			$xtpl->assign('width', $pro_config['homewidth']);
			$xtpl->assign('hometext', $data_row['hometext']);
			$xtpl->assign('num', $num_row );
			$xtpl->assign('CAT', $data_row['cat_title']);
			$xtpl->assign('CAT_LINK', $data_row['cat_link']);			
			...
			if( $pro_config['active_price'] == '1' ){
				if( $data_row['showprice'] == '1' ){					
					$xtpl->assign('product_price', nice_number( $data_row['product_price']) . ' ' . $data_row['money_unit'] .'/' . $data_row['product_unit']);
					$xtpl->parse('main.items.price' );
					$xtpl->parse('main.items.price1' );
				} else {
					$xtpl->parse('main.items.contact' );
					$xtpl->parse('main.items.contact1' );
				}
			}
			if( $pro_config['active_tooltip'] == 1 ) $xtpl->parse('main.items.tooltip' );
...
{
	global $module_info, $lang_module, $module_file, $module_name, $pro_config, $array_displays, $db, $db_config;

	$xtpl = new XTemplate( "view_cat.tpl", NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/" . $module_file );
	$xtpl->assign('LANG', $lang_module );
	$xtpl->assign('module_name', $module_file );
	$xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL );
	$xtpl->assign('NV_LANG_VARIABLE', NV_LANG_VARIABLE );
	...
	if( ! empty( $data_content['data']) ) {
		foreach( $data_content['data'] as $data_row ) {
			...
			$xtpl->assign('link_pro', $data_row['link_pro']);
			$xtpl->assign('img_pro', $data_row['homeimgthumb']);
			$xtpl->assign('img_pro_large', $data_row['homeimgfile']);				
			$xtpl->assign('link_order', $data_row['link_order']);
			$xtpl->assign('intro', $data_row['hometext']);
			$xtpl->assign('product_code', $data_row['product_code']);
			$xtpl->assign('hitstotal', $data_row['hitstotal']);			
/*
			$db->select('t1.bid, t1.' . NV_LANG_DATA . '_title As title, t1.' . NV_LANG_DATA . '_alias As alias')
				->from($db_config['prefix'] . '_' . $module_name . '_real_block t1')	
				->join('INNER JOIN ' . $db_config['prefix'] . '_' . $module_name . '_block t2 ON t2.bid = t1.bid')	
				->where('t2.id = ' . $data_row['id']);			
			$_result = $db->query($db->sql());
		
			foreach( $_result as $data_real_block_i ) {
				$xtpl->assign('REAL_BLOCK', $data_real_block_i['title']);
				$xtpl->assign('REAL_BLOCK_LINK', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $module_info['alias']['real_block'] . '/' . $data_real_block_i['alias']);
				$xtpl->parse('main.row.real_block');	
				$xtpl->parse('main.row.real_block1');	
			}												
*/
			
			if (!empty($data_row['real_type']) and !empty($data_row['real_type_link'])) {			
				$xtpl->assign('REAL_TYPE', $data_row['real_type']);
				$xtpl->assign('REAL_TYPE_LINK', $data_row['real_type_link']);
				$xtpl->parse('main.row.real_type');
				$xtpl->parse('main.row.real_type1');	
			}
			
			if (!empty($data_row['authorid'])) {				
					$xtpl->assign('author', $data_row['authorid']);	
					$xtpl->parse('main.row.author' );
					$xtpl->parse('main.row.author1' );
			}
...
//			$xtpl->assign('publtime', $lang_module['detail_dateup'] . " " . nv_date('d-m-Y h:i:s A', $data_row['publtime']) );
			...
			if( ! empty( $pro_config['show_product_code']) and ! empty( $data_row['product_code']) ) {
				...
				$xtpl->parse('main.row.product_code1' );
			}
 ...
 * @return
 */
function detail_product($data_content, $data_unit, $data_others, $data_shop, $array_other_view, $content_comment)
{
	global $module_info, $lang_module, $module_file, $module_upload, $module_name, $my_head, $pro_config, $money_config, $global_config, $detail_layout, $nv_Cache, $db, $db_config, $global_array_cat, $client_info;
	...
	$xtpl = new XTemplate($detail_layout, NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/" . $module_file );
...
//		$xtpl->assign('DATE_UP', $lang_module['detail_dateup'] . " " . nv_date('d-m-Y h:i:s A', $data_content['publtime']) );
		$xtpl->assign('DATE_UP', nv_date('d-m-Y', $data_content['publtime']) );
		...
		$xtpl->assign('CATLINK', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_cat[$catid]['alias']);	
        $xtpl->assign('ADDRESS', $data_content['caddress']);
		
		$db->sqlreset()
			->select('t1.bid, t1.' . NV_LANG_DATA . '_title As title, t1.' . NV_LANG_DATA . '_alias As alias, t2.bid, t2.id')
			->from($db_config['prefix'] . '_' . $module_name . '_real_block t1')	
			->join('INNER JOIN ' . $db_config['prefix'] . '_' . $module_name . '_block t2')	
			->where('t2.bid = t1.bid AND t2.id = ' . $data_content['id']);
		$list2 = $nv_Cache->db($db->sql(), '', $module_name);
		if (!empty($list2)) {
			$t = sizeof($list2) - 1;
			foreach ($list2 as $i => $value) {
				$xtpl->assign('REAL_BLOCK', $value['title']);
				$xtpl->assign('REAL_BLOCK_LINK', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=real_block/' . $value['alias']);
				$xtpl->assign('SLASH', ($t == $i) ? '' : ', ');
				$xtpl->parse('main.real_block.loop');
				$xtpl->parse('main.real_block1.loop1');
			}	
			$xtpl->parse('main.real_block');	
			$xtpl->parse('main.real_block1');			
		}			
		
		if( !empty($data_content[NV_LANG_DATA . '_realtitle']) ){
			$xtpl->assign('realtitle', $data_content[NV_LANG_DATA . '_realtitle']);
			$xtpl->parse('main.realtitle' );
		}	

		if( !empty($data_content[NV_LANG_DATA . '_realinfo']) ){
			$xtpl->assign('realinfo', $data_content[NV_LANG_DATA . '_realinfo']);
			$xtpl->parse('main.realinfo' );
		}	
		
		if( ! empty( $data_content[NV_LANG_DATA . '_design']) ) {
			$xtpl->assign('design', $data_content[NV_LANG_DATA . '_design']);
			$xtpl->parse('main.design' );
			$xtpl->parse('main.design1' );
		}		
		...
		if( ! empty( $data_content[NV_LANG_DATA . '_policy']) ) {
			$xtpl->assign('policy', $data_content[NV_LANG_DATA . '_policy']);
			$xtpl->parse('main.policy' );
		}
		
		if( ! empty( $data_content[NV_LANG_DATA . '_utility']) ) {
			$xtpl->assign('utility', $data_content[NV_LANG_DATA . '_utility']);
			$xtpl->parse('main.utility' );
		}
		
		
		if( ! empty($data_content['real_type']) and ($data_content['real_type_alias']) ) {
			$xtpl->assign('REAL_TYPE', $data_content['real_type']);
			$xtpl->assign('REAL_TYPE_LINK', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . '=real_type/' . $data_content['real_type_alias']);
			$xtpl->parse('main.real_type' );
		}
		
		if( $pro_config['active_price'] == '1' ) {
			if( $data_content['showprice'] == '1' ) {
				$xtpl->assign('product_price', nice_number( $data_content['product_price']) . ' ' . $data_content['money_unit'] .'/' . $data_content['product_unit']);
		...
		$support =  $data_content[NV_LANG_DATA . '_support'];
		if ( !empty ($support) ) {
			$xtpl->assign('support', $support );
		...
		// Property attribute		
		if(!empty($data_content['propatt'])) {
			$propatt = explode( '|', $data_content['propatt'] );
			
			if ($propatt[0] != "") {
				$xtpl->assign('prop_area', $propatt[0]);
				$xtpl->parse('main.propatt.prop_area');				
			}
			if ($propatt[1] != "") {
				$xtpl->assign('prop_bedr', $propatt[1]);
				$xtpl->parse('main.propatt.prop_bedr');				
			}			
			if ($propatt[2] != "") {
				$xtpl->assign('prop_floor', $propatt[2]);
				$xtpl->parse('main.propatt.prop_floor');				
			}	
			if ($propatt[3] != "") {
				$xtpl->assign('prop_front', $propatt[3]);
				$xtpl->parse('main.propatt.prop_front');				
			}
			if ($propatt[4] != "") {
				$xtpl->assign('prop_wc', $propatt[4]);
				$xtpl->parse('main.propatt.prop_wc');				
			}
			if ($propatt[5] != "") {
				$xtpl->assign('prop_dir', $propatt[5]);
				$xtpl->parse('main.propatt.prop_dir');				
			}	
			
			$xtpl->parse('main.propatt');
		}	
		
		// Gmap		
		if(!empty($data_content['gmap'])) {
			$gmap = explode( '|', $data_content['gmap'] );
			$xtpl->assign('GMAP_AD', $gmap[0]);	
			$xtpl->assign('GMAP_CL', $gmap[1]);
			$xtpl->assign('GMAP_CN', $gmap[2]);
			$xtpl->assign('GMAP_L', $gmap[3]);
			$xtpl->assign('GMAP_N', $gmap[4]);	
			$xtpl->assign('GMAP_Z', $gmap[5]);	
			$xtpl->parse('main.gmap');
			$xtpl->parse('main.gmap1');		
			$xtpl->parse('main.gmap_view');	
		}				
	
		if(!empty($data_content['street'])) {
			$street = explode( ',', $data_content['street'] );
			$xtpl->assign('STREET_CL', $street[0]);
			$xtpl->assign('STREET_CN', $street[1]);
			$xtpl->parse('main.street_view');
			$xtpl->parse('main.street_view1');			
		}	
		
		// Hien thi otherimage
		$otherimage = $data_content['otherimage'];
		$otherimage = explode('|', $otherimage);	
		if (!empty($otherimage)) {							
			foreach ($otherimage as $stt_i => $otherimage_i) {
				if (!empty($otherimage_i) and file_exists(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $otherimage_i)) {
					$otherimage_i = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $otherimage_i;
				} else {
					$otherimage_i = $otherimage_i;
				}
					$xtpl->assign('IMG_SRC_OTHER', $otherimage_i);
					$xtpl->assign('STT', $stt_i+1);
					$xtpl->parse('main.othersimg.loop');					
			}
			$xtpl->parse('main.othersimg');
		}	
		...
		if( !empty($data_content['author']) and !empty($data_content['author_number']) ) {
			$xtpl->assign('author', $data_content['author']);	
			$xtpl->assign('author_number', $data_content['author_number']);	
			$xtpl->assign('author_email', $data_content['author_email']);				
			if( !empty($data_content['author_photo']) ) {
				$author_photo = NV_BASE_SITEURL . $data_content['author_photo'];
			} else {
				$author_photo = NV_BASE_SITEURL . 'themes/' . $module_info['template'] . '/images/no_image.gif';
			}	
			$xtpl->assign('author_photo', $author_photo);
			$xtpl->parse('main.author');
			$xtpl->parse('main.author1');
			$xtpl->parse('main.author2');
		}	
		if( !empty($data_content['author']) and !empty($data_content['author_number']) ) {
			if(!empty($data_content['facebook'])) {
					$xtpl->assign('facebook', $data_content['facebook']);	
					$xtpl->parse('main.author3.facebook');
			}	
			if(!empty($data_content['twitter'])) {
					$xtpl->assign('twitter', $data_content['twitter']);	
					$xtpl->parse('main.author3.twitter');
			}		
			if(!empty($data_content['google_plus'])) {
					$xtpl->assign('google_plus', $data_content['google_plus']);	
					$xtpl->parse('main.author3.google_plus');
			}
			if(!empty($data_content['youtube'])) {
					$xtpl->assign('youtube', $data_content['youtube']);	
					$xtpl->parse('main.author3.youtube');
			}
			$xtpl->parse('main.author3');
		}
	...
	if($data_content['allowed_print'] == 1) {
	...
	if( $data_content['allowed_comm'] != 0 ) {
		$xtpl->assign('COMMENT', $content_comment);
		$xtpl->parse('main.comment');
	}
	
	if( !empty($data_content['layout_func']) ){
		$xtpl->assign('layout_func', $data_content['layout_func']);		
	}
 ...
 * real_block_theme()
 *
 * @param mixed $real_block_array
 * @param mixed $generate_page
 * @param mixed $page_title
 * @param mixed $description
 * @param mixed $real_block_image
 * @return
 */
function real_block_theme($real_block_array, $generate_page, $page_title, $description, $real_block_image, $sort = 0)
{
    global $lang_module, $module_info, $module_name, $module_config, $bid, $array_displays, $base_url_rewrite, $pro_config, $nv_Cache, $db, $db_config, $global_array_cat, $product_unit;

    $xtpl = new XTemplate('real_block.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_info['module_theme']);
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('REAL_BLOCK_TITLE', $page_title);
    $xtpl->assign('REAL_BLOCK', $base_url_rewrite);	
	
    if ($bid and !empty($real_block_array)) {
		...
		if($pro_config['show_displays'] == 1) {
			foreach($array_displays as $k => $array_displays_i) {
					...
					$se = ($sort == $k) ? 'selected="selected"' : '';
					$xtpl->assign('se', $se);
					$xtpl->parse('main.row.displays.sorts');
				}
				$xtpl->parse('main.row.displays');
		}	
		
        foreach ($real_block_array as $real_block_array_i) {
            $xtpl->assign('ROW', $real_block_array_i);
            $xtpl->assign('TIME', date('H:i', $real_block_array_i['publtime']));
            $xtpl->assign('publtime', date('d/m/Y', $real_block_array_i['publtime']));	
            $xtpl->assign('CAT', $global_array_cat[$real_block_array_i['listcatid']]['title']);			
            $xtpl->assign('CATLINK', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_cat[$real_block_array_i['listcatid']]['alias']);					
			
            if ($bid and defined('NV_IS_MODADMIN')) {
                $xtpl->assign('ADMINLINK', nv_link_edit_page($real_block_array_i['id']) . ' ' . nv_link_delete_page($real_block_array_i['id']));
                $xtpl->parse('main.row.loop.adminlink');
            }		
			
			if (!empty($real_block_array_i['id'])) {
			$db->sqlreset()
				->select('rid, ' . NV_LANG_DATA . '_title As title, ' . NV_LANG_DATA . '_alias As alias')
				->from($db_config['prefix'] . '_' . $module_name . '_real_type')	
				->where('rid = ' . $real_block_array_i['r_id']);
			$list2 = $nv_Cache->db($db->sql(), '', $module_name);
				if (!empty($list2)) {
					foreach ($list2 as $l2) {
						$l2['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=real_type/' . $l2['alias'];
						$xtpl->assign('REAL_TYPE', $l2);
					$xtpl->parse('main.row.loop.real_type');
					$xtpl->parse('main.row.loop.real_type1');						
					}		
				}	
			}
			
			if(!empty($pro_config['show_product_code']) and !empty($real_block_array_i['product_code'])) {
				$xtpl->parse('main.row.loop.product_code' );
				$xtpl->parse('main.row.loop.product_code1' );
			}			
			
			if (!empty($real_block_array_i['auth'])) {				
					$xtpl->assign('author', $real_block_array_i['auth']);	
					$xtpl->parse('main.row.loop.author' );
					$xtpl->parse('main.row.loop.author1' );
			}
			
			if( $pro_config['active_price'] == '1' ) {
				if( $real_block_array_i['showprice'] == '1' ) {
					$xtpl->assign('product_price', nice_number( $real_block_array_i['product_price']) . ' ' . $real_block_array_i['money_unit'] .'/' . $product_unit );
					$xtpl->parse('main.row.loop.price' );
					$xtpl->parse('main.row.loop.price1' );
				} else {
					$xtpl->parse('main.row.loop.contact' );
					$xtpl->parse('main.row.loop.contact1' );
				}
			}	
		...
		foreach ($real_block_array as $real_block_array_i) {
			if ($real_block_array_i['numrow'] > 0){
				$xtpl->assign('NUMROW', $real_block_array_i['numrow']);
				$xtpl->assign('REAL_BLOCK', $real_block_array_i);
				$xtpl->assign('DATE', date('d/m/Y', $real_block_array_i['publtime']));
				if (! empty($real_block_array_i['image'])) {
					$xtpl->assign('REAL_BLOCK_IMG', $real_block_array_i['image']);
					$xtpl->parse('main.real_block.row.image');
				}	
				$xtpl->parse('main.real_block.row');
			}
		}	
		$xtpl->parse('main.real_block');		
	}
 ...
 * real_type_theme()
 *
 * @param mixed $real_type_array
 * @param mixed $generate_page
 * @param mixed $page_title
 * @param mixed $description
 * @param mixed $real_type_image
 * @return
 */
function real_type_theme($real_type_array, $generate_page, $page_title, $real_type_image, $sort = 0)
{
    global $lang_module, $module_info, $module_name, $module_config, $rid, $array_displays, $base_url_rewrite, $pro_config, $nv_Cache, $db, $db_config, $global_array_cat, $numrow, $product_unit;

    $xtpl = new XTemplate('real_type.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_info['module_theme']);
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('REAL_TYPE_TITLE', $page_title);
    $xtpl->assign('REAL_TYPE_LINK', $base_url_rewrite);	
	
    if ($rid and ! empty($real_type_array)) {
        ...
        foreach ($real_type_array as $real_type_array_i) {
            $xtpl->assign('ROW', $real_type_array_i);
            $xtpl->assign('TIME', date('H:i', $real_type_array_i['publtime']));
            $xtpl->assign('publtime', date('d/m/Y', $real_type_array_i['publtime']));	
            $xtpl->assign('CAT', $global_array_cat[$real_type_array_i['listcatid']]['title']);			
            $xtpl->assign('CATLINK', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_cat[$real_type_array_i['listcatid']]['alias']);				

            if ($rid and defined('NV_IS_MODADMIN')) {
                $xtpl->assign('ADMINLINK', nv_link_edit_page($real_type_array_i['id']) . ' ' . nv_link_delete_page($real_type_array_i['id']));
                $xtpl->parse('main.row.loop.adminlink');
            }	

/*			
			foreach( $real_type_array_i['data_real_block'] as $data_real_block_i ) {
				$xtpl->assign('real_block', $data_real_block_i['real_block']);
				$xtpl->assign('real_block_LINK', $data_real_block_i['real_block_link']);
				$xtpl->parse('main.row.loop.real_block');	
				$xtpl->parse('main.row.loop.real_block1');	
			}
*/			
			
			if(!empty($pro_config['show_product_code']) and !empty($real_type_array_i['product_code'])) {
				$xtpl->parse('main.row.loop.product_code' );
				$xtpl->parse('main.row.loop.product_code1' );
			}			
			
			if (!empty($real_type_array_i['auth'])) {				
					$xtpl->assign('author', $real_type_array_i['auth']);	
					$xtpl->parse('main.row.loop.author' );
					$xtpl->parse('main.row.loop.author1' );
			}
			
			if( $pro_config['active_price'] == '1' ) {
				if( $real_type_array_i['showprice'] == '1' ) {
					$xtpl->assign('product_price', nice_number( $real_type_array_i['product_price']) . ' ' . $real_type_array_i['money_unit'] .'/' . $product_unit );
					$xtpl->parse('main.row.loop.price' );
					$xtpl->parse('main.row.loop.price1' );
				} else {
					$xtpl->parse('main.row.loop.contact' );
		...
		foreach ($real_type_array as $real_type_array_i) {		
			if ($real_type_array_i['numrow'] > 0){
				$xtpl->assign('NUMROW', $real_type_array_i['numrow']);
				$xtpl->assign('REAL_TYPE', $real_type_array_i);
				$xtpl->assign('DATE', date('d/m/Y', $real_type_array_i['publtime']));
				
				if (! empty($real_type_array_i['image'])) {
					$xtpl->assign('REAL_TYPE_IMG', $real_type_array_i['image']);
					$xtpl->parse('main.real_type.image.row');
				}	
				$xtpl->parse('main.real_type.row');
			}			
		}	
		$xtpl->parse('main.real_type');			
	}
