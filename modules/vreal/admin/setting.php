...
// Tien te
$result = $db->query( "SELECT currency, code  FROM " . $db_config['prefix'] . "_" . $module_data . "_money_" . NV_LANG_DATA . " ORDER BY code DESC" );
while( list( $currency, $code ) = $result->fetch( 3 ) )
{
	$array_temp = array();
	$array_temp['value'] = $currency;
	$array_temp['title'] =  $currency. " - " . $code;
	$array_temp['selected'] = ( $currency == $data['money_unit'] ) ? " selected=\"selected\"" : "";
