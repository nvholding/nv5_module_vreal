...
if (! nv_function_exists('vreal_sidebar_block')) {
    function block_config_vreal_sidebar($module, $data_block, $lang_block)
    {
    ...
    function block_config_vreal_sidebar_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function vreal_sidebar_block($block_config)
    {
		...
		if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/vreal/block_vreal_sidebar.tpl' ) ){
			$block_theme = $global_config['module_theme'];
		} else {
			$block_theme = 'default';
		}
			$xtpl = new XTemplate( 'block_vreal_sidebar.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/vreal' );
		...
		if($module_name != 'vreal'){
			function nice_number($n) {
				...
				// now filter it;
				if ($n >= 1000000000000) return round(($n/1000000000000), 2) . ' ' .$lang_module['trillion'];
				elseif ($n >= 1000000000) return round(($n/1000000000), 2) . ' ' . $lang_module['billion'];
				elseif ($n >= 1000000) return round(($n/1000000), 2) . ' ' . $lang_module['million'];
				elseif ($n >= 1000) return round(($n/1000), 2) . ' ' . $lang_module['thousand'];
...
if (defined('NV_SYSTEM')) {
    $content = vreal_sidebar_block($block_config);
}
