...
if (!function_exists('vreal_search_product')) {

    /**
     * vreal_search_product()
     *
     * @param mixed $block_config
     * @return
     *
     */
    function vreal_search_product($block_config)
    {
		...
		$raid = $nv_Request->get_int( 'rida', 'get', 0 );
        ...
        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/block_vreal_search.tpl')) {
        ...
        if ($module != $module_name) {
            $my_head .= '<script type="text/javascript" src="' . NV_BASE_SITEURL . 'modules/' . $mod_file . '/js/vreal.js"></script>';
        }

        $xtpl = new XTemplate('block_vreal_search.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
        ...
        // Get money
        $sql = 'SELECT id, code, currency FROM ' . $db_config['prefix'] . '_' . $mod_data . '_money_' . NV_LANG_DATA . ' ORDER BY id ASC';
        $list = $nv_Cache->db($sql, '', $module);
		...
		$sql = 'SELECT rid, ' . NV_LANG_DATA . '_title AS title FROM ' . $db_config['prefix'] . '_' . $mod_data . '_real_type ORDER BY rid ASC';
		$list = $nv_Cache->db( $sql, '', $module );
...
if (defined('NV_SYSTEM')) {
    $content = vreal_search_product($block_config);
}
