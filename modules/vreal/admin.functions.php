...
$allow_func = array( 'main', 'alias', 'items', 'exptime', 'publtime', 'setting', 'content', 'keywords', 'del_content', 'cat', 'change_cat', 'list_cat', 'del_cat', 'real_type', 'change_real_type', 'list_real_type', 'del_real_type', 'real_block', 'del_real_block', 'list_real_block', 'chang_real_block', 'prounit', 'delunit', 'order', 'or_del', 'or_view', 'money', 'delmoney', 'active_pay', 'payport', 'changepay', 'actpay', 'docpay', 'group', 'del_group', 'list_group', 'change_group', 'getcatalog', 'getgroup' );

$array_viewcat_full = array(
	'view_home_cat' => $lang_module['view_home_cat'],
	'view_home_catlist' => $lang_module['view_home_catlist'],
 ...
 * @return
 */
function nv_fix_real_block()
{
	global $db, $db_config, $module_data;

	$sql = 'SELECT bid FROM ' . $db_config['prefix'] . '_' . $module_data . '_real_block ORDER BY weight ASC';
	$weight = 0;
	$result = $db->query( $sql );
	while( $row = $result->fetch() )
	{
		++$weight;
		$sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_real_block SET weight=' . $weight . ' WHERE bid=' . $row['bid'];
		$db->query( $sql );
	}
	$result->closeCursor();
}
 ...
 * @return
 */
function nv_fix_real_type()
{
	global $db, $db_config, $module_data;

	$sql = 'SELECT rid FROM ' . $db_config['prefix'] . '_' . $module_data . '_real_type ORDER BY weight ASC';
	$result = $db->query( $sql );
	$weight = 0;
	while( $row = $result->fetch() )
	{
		++$weight;
		$sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_real_type SET weight=' . $weight . ' WHERE rid=' . $row['rid'];
		$db->query( $sql );
	}
	$result->closeCursor();
}
...
/**
 * nv_show_real_block_list()
 *
 * @return
 */
function nv_show_real_block_list()
{
	global $db, $db_config, $lang_module, $lang_global, $module_name, $module_data, $op, $global_config, $module_file;

	$xtpl = new XTemplate( "real_block_list.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_file );
	$xtpl->assign( 'LANG', $lang_module );
	$xtpl->assign( 'GLANG', $lang_global );
	$xtpl->assign( 'NV_BASE_ADMINURL', NV_BASE_ADMINURL );
	$xtpl->assign( 'NV_NAME_VARIABLE', NV_NAME_VARIABLE );
	$xtpl->assign( 'NV_OP_VARIABLE', NV_OP_VARIABLE );
	$xtpl->assign( 'MODULE_NAME', $module_name );
	$xtpl->assign( 'OP', 'real_block' );

	$sql = "SELECT * FROM " . $db_config['prefix'] . "_" . $module_data . "_real_block ORDER BY weight ASC";
	$result = $db->query( $sql );
...
/**
 * nv_show_real_type_list()
 *
 * @return
 */
function nv_show_real_type_list()
{
	global $db, $db_config, $lang_module, $lang_global, $module_name, $module_data, $op, $nv_Request, $global_config, $module_file;

	$xtpl = new XTemplate( "real_type_list.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_file );
	$xtpl->assign( 'LANG', $lang_module );
	$xtpl->assign( 'GLANG', $lang_global );
	$xtpl->assign( 'NV_BASE_ADMINURL', NV_BASE_ADMINURL );
	$xtpl->assign( 'NV_NAME_VARIABLE', NV_NAME_VARIABLE );
	...
	$num = $db->query( "SELECT * FROM " . $db_config['prefix'] . "_" . $module_data . "_real_type ORDER BY weight ASC" )->rowCount();
	$base_url = NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=real_type";
	...
	{
		$a = 0;
		$db->sqlreset()->select( '*' )->from( $db_config['prefix'] . "_" . $module_data . "_real_type" )->order( 'weight' )->limit( $per_page )->offset( ( $page - 1 ) * $per_page );
