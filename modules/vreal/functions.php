...
{
	global $global_array_cat, $module_name, $db, $db_config, $module_data, $link, $module_info, $global_config;
	$data_content = array();
	$data = array();
	while( list( $id, $listcatid, $r_id, $user_id, $publtime, $title, $alias, $hometext, $address, $homeimgalt, $homeimgfile, $homeimgthumb, $product_code, $product_price, $product_discounts, $money_unit, $showprice, $product_unit, $author, $hitstotal ) = $result->fetch( 3 ) )
	{
		...
		$sql = $db->query( 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_real_type WHERE rid = ' . $r_id );
		$data_real_type = $sql->fetch();
		$real_type = $data_real_type[NV_LANG_DATA . '_title'];
		$real_type_link = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $module_info['alias']['real_type'] . '/' . $data_real_type[NV_LANG_DATA . '_alias'];
			
		if (!empty($author)){
			$sql = $db->query('SELECT userid, first_name, last_name FROM ' . $db_config['prefix'] . '_users WHERE userid = ' . $user_id );
			$data_author = $sql->fetch();	
			$_author = $data_author['first_name'] . ' ' . $data_author['last_name'];
		}
